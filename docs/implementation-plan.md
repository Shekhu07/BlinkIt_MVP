...
- [x] **Enhancement (2026-07-24): two extra strategy surfaces added to the live app as tabs** (both clearly-labelled synthetic simulations; the operator console is unchanged as tab 1):
  - **Auto-nudge queue (targeting/automation layer)** — `mvp/auto_targeting.py`. Selects the cohort that would *automatically* receive a "try a new category" push (vs. an operator hand-picking): **order_frequency = Weekly AND tenure > 6 months** (SYN-004/005/008 of the 8 synthetic users). A "Run scheduled batch" button generates, live via Groq, the notification each eligible user would get. Deterministic eligibility; **no real push is sent** (labelled as such — there is no live user base).
