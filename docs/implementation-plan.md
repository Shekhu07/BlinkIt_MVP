...
- [x] **Enhancement (2026-07-24): peer social-proof line added to the nudge** — maps to the "Sentiment-Targeted Social Proof" pillar of the external Category-Expansion playbook and to Q15's item-reviews/ratings driver (5/31 — tied *third* at the N=25 rollup, tied *fourth* after the 2026-07-28 recount; the line remains justified, but must not be described as third-ranked). Implemented as a third trust line beside refund + freshness (`social_proof_line` in `agent.py`, rendered on the phone card and PM panel in `app.py`). Stays inside the single-nudge guardrail (a line, not a new mechanic). **Hard constraint enforced in the prompt: no fabricated numbers/ratings/counts** — the playbook's own examples ("Over 1,200 buyers rated 5/5") are invented and were NOT imported; the line is qualitative peer reassurance only, and is empty for out-of-primary-scope (low-intent) users. Verified live across all 8 profiles: zero digits in any social-proof line, empty on the two out-of-scope profiles.
- [x] **Enhancement (2026-07-24): two extra strategy surfaces added to the live app as tabs** (both clearly-labelled synthetic simulations; the operator console is unchanged as tab 1):
  - **Auto-nudge queue (targeting/automation layer)** — `mvp/auto_targeting.py`. Selects the cohort that would *automatically* receive a "try a new category" push (vs. an operator hand-picking): **order_frequency ∈ {Daily, Weekly} AND tenure > 6 months** (5 of the 8 synthetic users: SYN-001/004/005/006/008). A "Run scheduled batch" button generates, live via Groq, the notification each eligible user would get. Deterministic eligibility; **no real push is sent** (labelled as such — there is no live user base).
  - **Checkout cart-filler (playbook Pillar 4 — now built, was backlog)** — `mvp/cart_filler.py`. When a cart is short of the ₹199 free-delivery threshold, offers a low-cost item from a category the user has **never bought**, turning the threshold top-up into a zero-friction new-category trial. Deterministic, **no LLM** (protects Groq quota); `FILLER_CATALOG` is synthetic/labelled, prices illustrative. Strategically aimed at the **low-intent/no-incident** segment the push nudge deliberately skips (a cheap add needs no pre-existing intent). Never-bought categories are ranked by the same basket-adjacency ranker so fillers feel relevant.
  - **Scope note:** these add mechanics beyond the original single-nudge guardrail — a deliberate product decision (2026-07-24), delivered as demonstrations, not a claim that they're production-validated. The pre-acceptance-inspection driver remains the one still-deferred backlog item.
...
- [x] **Post-launch UI pass (2026-07-23)**: both live apps redesigned from Claude Design projects into one shared visual language (see `architecture.md` §6.4). The MVP is now an operator console + phone mockup; the discovery app is a console (two tabs at the time of this pass, three since the Bulk Run addition below). In both, the mockups' invented numbers/copy were replaced with the real pipeline export and live LLM output — substitutions tabulated in `mvp/README.md` and `discovery/README.md`. `agent.py`'s output schema was extended (headline/body/refund_line/fresh_line/cta/product/why_user/why_category) with the five hard rules unchanged; `data/synthetic_profiles.json` gained display-only fields, still labelled SYNTHETIC. Verified headlessly at the time: identical light/dark render, no mobile overflow. **The "0 WCAG AA contrast failures" claim originally recorded here did not hold** — a full walk of all three discovery tabs on 2026-07-30 found 11 items under 3:1 and the MVP found 7, some pre-existing (inline gold icons, funnel arrows, pale legend swatches) and some caused by the Gradio 6 CSS-specificity regression (see `architecture.md` §6.4). All are now fixed and re-measured live at 0 under 3:1, excluding HuggingFace's own injected `#huggingface-space-header` chrome, which app CSS cannot reach. The original check evidently sampled only the first tab; audit every tab.
