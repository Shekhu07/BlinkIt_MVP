...
/* cart-flow add-to-cart buttons */
.nb-add-btn.nb-add-btn{width:100% !important;border:1.5px solid #EFEFE9 !important;
  font-size:12.5px !important;font-weight:700 !important;padding:10px 12px !important;
  border-radius:12px !important;color:#16130A !important;background:#fff !important;
  text-align:left !important;margin-bottom:8px !important}
.nb-add-btn.nb-add-btn:hover{border-color:#F8CD1B !important}
...
<div style="border:1.5px dashed #DADBD3;border-radius:20px;padding:44px 20px;text-align:center">
  ...
  <div style="font-size:13px;color:#63635A;margin-top:10px;line-height:1.5;font-weight:500">
    ...
    # as "Kitchen &Amp; Dining". Hits every category containing "&".
...
def _flow_placeholder_status():
    return ('<div class="nb-eyebrow">Step 2 · Operator console</div>'
            '<div style="font-size:13px;color:#6B6B60;margin-top:6px;line-height:1.5">'
            'Add an item on the left to trigger the next push nudge.</div>')


def flow_refresh(uid, cart_total):
    """Redraws the Step 1 checkout screen and syncs the (up to 3) real "Add" buttons
    to whatever suggest_fillers() currently offers for this user/cart-total."""
    p = BY_ID[uid]
    cart_total = int(cart_total)
    res = suggest_fillers(p, cart_total)
    items = res["items"]
    ...
    for i in range(3):
        if i < len(items):
            ...
            btn_updates.append(gr.update(
                value=f'🛍️ Add "{it["name"]}" · {it["category"]} (₹{it["price"]})', visible=True))
        else:
            btn_updates.append(gr.update(visible=False))
    return (cart_phone_html(uid, cart_total), *btn_updates, items)


def flow_reset_right(uid):
    return _flow_placeholder_status(), "", phone_html(BY_ID[uid])


def _added_item_trust_html(item):
    """Deterministic, no-LLM reassurance for the item just added via the cart-filler —
    the same two top-ranked trust drivers the push-nudge agent leads with (refund
    guarantee + a visible quality/freshness signal), applied to what the shopper just
    put in their cart rather than invented per-item. Kept template-only (no Groq call):
    the cart-filler layer is deliberately LLM-free (see cart_filler.py's module docstring)."""
    return f"""
<div class="nb-dark" style="margin-top:10px">
  <div class="k">Added to cart</div>
  <div class="h">{esc(item["name"])} · {esc(item["category"])}</div>
  <div style="margin-top:8px;font-size:13px;line-height:1.6">
    🔄 No-questions-asked refund if it isn't right<br>
    🛡️ Verified brand tag — sealed and freshness-checked
  </div>
</div>"""


def flow_add_item(idx, uid, items):
    if idx >= len(items):
    ...
    p = BY_ID[uid]
    theme, reason = match_profile_to_theme(p, THEMES)
    trust_html = _added_item_trust_html(item)
    if "GROQ_API_KEY" not in os.environ:
        err = (_flow_placeholder_status().replace(
            "Add an item on the left to trigger the next push nudge.",
            "GROQ_API_KEY is not configured on the server. Add it in the Space Settings "
            "→ Variables and secrets.") + trust_html)
        return err, "", phone_html(p)
    try:
        r = generate_nudge(p, theme, reason, exclude_category=category)
    except Exception as e:  # noqa: BLE001
        err = (_flow_placeholder_status().replace(
            "Add an item on the left to trigger the next push nudge.", esc(e)) + trust_html)
        return err, "", phone_html(p)
    status = (
        '<div class="nb-eyebrow">Step 2 · Operator console</div>'
        f'{trust_html}'
        f'<div style="font-size:13px;color:#44443B;margin-top:10px;line-height:1.5">'
        f'Excluded from this pick, so the push nudge below targets a different '
        f'never-bought category: <b>{esc(r.get("suggested_category", ""))}</b>.</div>')
    return status, reasoning_html(theme, r, p=p), phone_html(p, r)
...
with gr.Blocks(title="Blinkit Cart -> Nudge Flow", css=CSS, head=FONT_LINK,
               js=FORCE_LIGHT, theme=gr.themes.Base()) as demo:
    gr.HTML(FONT_LINK)
    gr.HTML(TOP)

    with gr.Tabs():
        with gr.Tab("Cart → Nudge flow"):
            gr.HTML('<div class="nb-eyebrow">Step 1 → Step 2, chained</div>'
                    '<div style="font-size:13px;color:#44443B;margin:6px 0 14px;line-height:1.5;max-width:760px">'
                    'Add a checkout-filler item first. Once it lands in the cart, the operator console '
                    'generates the next push nudge for a <b>different</b> never-bought category — the '
                    'project\'s core thesis is a new category every month, not the same one twice.</div>')
            with gr.Row(elem_classes="nb-cols"):
                with gr.Column(scale=100):
                    with gr.Column(elem_classes="nb-card"):
                        gr.HTML('<div class="nb-eyebrow" style="margin-bottom:8px">Step 1 · Add a cart-filler item</div>')
                        flow_user = gr.Dropdown(
                            choices=[(p["display_name"], p["user_id"]) for p in PROFILES],
                            value=PROFILES[0]["user_id"], label="User")
                        flow_amt = gr.Slider(0, FREE_DELIVERY_THRESHOLD + 60, value=150, step=5,
                                             label="Cart total (₹)")
                    with gr.Column(elem_classes="nb-card"):
                        flow_cart_out = gr.HTML(cart_phone_html(PROFILES[0]["user_id"], 150))
                        _flow_items0 = suggest_fillers(PROFILES[0], 150)["items"]

                        def _flow_btn_kwargs(i):
                            if i < len(_flow_items0):
                                it = _flow_items0[i]
                                return dict(value=f'🛍️ Add "{it["name"]}" · {it["category"]} (₹{it["price"]})',
                                            visible=True)
                            return dict(value="", visible=False)

                        flow_add0 = gr.Button(elem_classes="nb-add-btn", **_flow_btn_kwargs(0))
                        flow_add1 = gr.Button(elem_classes="nb-add-btn", **_flow_btn_kwargs(1))
                        flow_add2 = gr.Button(elem_classes="nb-add-btn", **_flow_btn_kwargs(2))
                with gr.Column(scale=110):
                    with gr.Column(elem_classes="nb-card"):
                        flow_status = gr.HTML(_flow_placeholder_status())
                    flow_reason_out = gr.HTML("")
                    flow_phone_out = gr.HTML(phone_html(PROFILES[0]))
            flow_items_state = gr.State(_flow_items0)

    flow_add_btns = (flow_add0, flow_add1, flow_add2)
    flow_amt.change(flow_refresh, inputs=[flow_user, flow_amt],
                    outputs=[flow_cart_out, *flow_add_btns, flow_items_state])
    flow_user.change(flow_refresh, inputs=[flow_user, flow_amt],
                     outputs=[flow_cart_out, *flow_add_btns, flow_items_state]
                     ).then(flow_reset_right, inputs=flow_user,
                            outputs=[flow_status, flow_reason_out, flow_phone_out])
    for _i, _btn in enumerate(flow_add_btns):
        _btn.click(lambda uid, cats, i=_i: flow_add_item(i, uid, cats),
                   inputs=[flow_user, flow_items_state],
                   outputs=[flow_status, flow_reason_out, flow_phone_out])
