...
- **Gradio is pinned to 4.44.1** (`sdk_version` in each README front-matter *and* `gradio==4.44.1` in each `requirements.txt`, in both the Spaces and `mvp/`/`discovery/`). Don't unpin: Gradio 6 stops auto-prefixing `css=` selectors with the container class, so every colour rule loses to `.prose *` and dark-panel text renders `#27272A` on `#16130A` (1.25:1, invisible). This shipped live once — see `docs/architecture.md` §6.4.
