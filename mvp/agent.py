...
# Groq free-tier quota can be exhausted by a burst of calls (e.g. a batch run). Retry
# transient/rate-limit failures with backoff, and cache identical calls so repeat
# clicks for the same user/theme don't re-spend quota.
_MAX_RETRIES = 4
_BASE_DELAY_S = 1.5
_NUDGE_CACHE = {}
...
def generate_nudge(profile, theme, match_reason, client=None):
    cache_key = (profile.get("user_id"), theme["name"], match_reason)
